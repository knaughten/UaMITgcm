...
C-----------------------------------------------------------------------
C     parameters for forced convection regions within subroutine "bldepth"
C
C     useFrcConv :: if true, set hbl to the seafloor in specified regions
C     frcConvMaskFile :: filename containing 2D mask which is 1 wherever
C                        the user wants to force convection, 0 otherwise
C     KPPfrcConvMask :: stores contents of that file
C-----------------------------------------------------------------------

      LOGICAL useFrcConv
      common /kpp_frcconv1/ useFrcConv
      CHARACTER*(MAX_LEN_FNAM) frcConvMaskFile
      common /kpp_frcconv2/ frcConvMaskFile
#ifdef ALLOW_FORCED_CONVECTION
      _RS KPPfrcConvMask(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      COMMON /kpp_frcconv3/ KPPfrcConvMask
#endif /* ALLOW_FORCED_CONVECTION */      
