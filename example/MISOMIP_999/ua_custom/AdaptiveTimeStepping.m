...
if CtrlVar.DefineOutputsDt(CtrlVar.DefineOutputsCounter)>0

    % Unlikely to be the case, but make sure dtOut is equal or smaller than Dt
    if dtOut>CtrlVar.DefineOutputsDt(CtrlVar.DefineOutputsCounter)
        dtOut=CtrlVar.DefineOutputsDt(CtrlVar.DefineOutputsCounter);
    end

    %temp=dtOut;
    %dtOut=NoOverStepping(CtrlVar,time,dtOutCopy,CtrlVar.DefineOutputsDt);
    dtNoOverStepping=NoOverStepping(CtrlVar,time,dtOut,CtrlVar.DefineOutputsDt(CtrlVar.DefineOutputsCounter));
    ...
    if dtNoOverStepping<dtOut  % so I may need to reduce dtOut, but maybe dtOut is only be a bit smaller than dtNoOverStepping
        ...
        T1=ReminderFraction(CtrlVar.time+dtOut,CtrlVar.DefineOutputsDt(CtrlVar.DefineOutputsCounter)) < (dtOut/(100*CtrlVar.DefineOutputsDt(CtrlVar.DefineOutputsCounter)));
        T2=ReminderFraction(CtrlVar.time+dtNoOverStepping,CtrlVar.DefineOutputsDt(CtrlVar.DefineOutputsCounter)) < (dtNoOverStepping/(100*CtrlVar.DefineOutputsDt(CtrlVar.DefineOutputsCounter))) ;
