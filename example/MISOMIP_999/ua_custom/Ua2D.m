...
%% Modified by Jan De Rydt on 13 Feb 2024 to work with Matlab RC and Ua-MITgcm
%% Original version: UaSource_beta_19Jan2024 @ c8f802a
...
%% JDR 13/02/2024: removed to make compatible with compiled Ua-MITgcm
% SetUaPath() %%
...
%% JDR 13/02/2024: output counter starts at 1 to make compatible with 
%% with variable Output interval
...
while 1
    ...
    elseif CtrlVar.UaRunType=="-uvh-"  ||  CtrlVar.UaRunType=="-uv-h-"
        ...
        if CtrlVar.UaRunType=="-uvh-"
            ...
            if CtrlVar.InitialDiagnosticStep   % if not a restart step, and if not explicitly requested by user, then do not do an inital dignostic step
                ...
                %% JDR 13/02/2024: removed to avoid increasing the outputs counter
                %% at the initial diagnostic step. In Ua-MITgcm the outputs 
                %% counter can only increase at times defined by the Ua-MITgcm
                %% calendar file, which are stored in CtrlVar.DefineOutputsDt
                %CtrlVar.DefineOutputsCounter = CtrlVar.DefineOutputsCounter + 1;
                ...
                %% JDR 13/02/2024: this situation should never be encountered
                %% in Ua-MITgcm, so the check was removed
                % if (ReminderFraction(CtrlVar.time,CtrlVar.DefineOutputsDt(CtrlVar.DefineOutputsCounter))<1e-5 || CtrlVar.DefineOutputsDt(CtrlVar.DefineOutputsCounter)==0 )
    ...
    %% JDR 13/02/2024: at various places here we account for the fact that 
    %% CtrlVar.DefineOutputsDt can be an array with different output times
    %% (in the default Ua configuration, this is always an integer)
    %% CtrlVar.DefineOutputsCounter is used to determine which element of 
    %% the array needs to be compared to the time
    if (ReminderFraction(CtrlVar.time,CtrlVar.DefineOutputsDt(CtrlVar.DefineOutputsCounter)) < (CtrlVar.dt/(10*CtrlVar.DefineOutputsDt(CtrlVar.DefineOutputsCounter)))) || CtrlVar.DefineOutputsDt(CtrlVar.DefineOutputsCounter)==0 
